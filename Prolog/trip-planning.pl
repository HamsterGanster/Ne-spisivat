...
% В общем trip(A,B) — дуга графа

% <YOUR CODE HERE>
% near(_, _, []) :- fail.
% near(_, _, [_]) :- fail.
% near(A, B, [A, B | _]).
% near(A, B, [X | Xs]) :- near(A, B, Xs).
...
% то есть в расписании  И  распиание имеет вид [_ | PT1,PT2 | _].
...
% <YOUR CODE HERE>
% tt is [arrive(P1 at T1), arrive(P2 at T2), ...]
from_to(PT, PT, []).
from_to(PT1, PT2, [PT1|Path]) :- 
    trip(PT1, Tmp), 
    from_to(Tmp, PT2, Path).
% His version:
from_to(PT, PT, []).
from_to(Start, Dest, [arrive(Next)|Path]) :- 
    trip(PT1, Next), 
    from_to(Next, PT2, Path).
from_to(CurrPlace at CurrTime, Dest, [stay(CurrPlace, CurrTime, NextTime) | Rs]) :- 
    trip(CurrPlace at NextTime, _), % существует выход отсюда, а не кладбище кораблей
    CurrTime before NextTime, % мы не опоздали на новый корабль
    schedule(CurrPlace at NextTime, Dest, Rs). % Ищем в новом расписании.
...
% <YOUR CODE HERE>
%My version:
% schedule(PT, PT, Schedule) :- tt(Schedule), memberchk(PT, Schedule), 
% His ver:
schedule(Start, Dest, [depart(Start), arrive(Next)|Rs]) :-
    ll(Rs, _), % "Сортировка" по длине расписания
    trip(Start, Next),
    from_to(Next, Dest, Rs).
