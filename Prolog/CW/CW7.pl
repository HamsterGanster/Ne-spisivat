 ...
 * 
 * An expert system not only simulates a resoning process of a human expert, but
...
% true(+Hypothesis, -Proof, +ProofTrace)
...
true(Conclusion, Conclusion <== ConditionProof, Trace) :-
    ...
    true(Cond, ConditionProof, [Conclusion <== ConditionProof | Trace]).


% ask user about observations
true(Observation, Proof, Trace) :- 
    askable(Observation),
    not(derived(Observation)),
    not(asked(Observation)),
    ask(Observation, Proof, Trace).


% Asking user to provide 

ask(Observation, Proof, Trace) :-
    % format("Pravda chto ~w dokazivaetca ~w po pravilam ~w: ", [Observation, Proof, Trace]),
    format("Is it true ~w. Input 'yes', 'no' or 'why': ", [Observation]),
    read_string(user_input, "\n", "\r\t", _, Answer),
    process(Answer, Observation, Proof, Trace).
